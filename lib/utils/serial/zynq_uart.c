...
static volatile Xil_Uart* uart;

static struct sbi_console_device zynq_console = {
	.name = "zynq_uart",
	.console_putc = xil_uart_putc,
	.console_getc = xil_uart_getc
};

bool xil_uart_init(unsigned long base_uart)
{
    uint32_t ret;
    uart = (volatile Xil_Uart*) base_uart;
    ret = xil_uart_set_baud_rate(UART_BAUD_RATE);
    if (ret == false) {
        return false;
    }
    ...
    sbi_console_set_device(&zynq_console);
    return true;
}

void xil_uart_enable()
{
...
bool xil_uart_set_baud_rate(uint32_t baud_rate)
{
...
void xil_uart_puts(const char* s)
{
    while (*s) {
        xil_uart_putc(*s++);
    }
...
void xil_uart_enable_irq(uint32_t irq){
	uart->isr_en = irq;
	uart->isr_mask |= irq;
}

void xil_uart_clear_irq(uint32_t irq){
	uart->isr_status = irq;
}

void xil_uart_clear_rxbuf(){
	while(uart->ch_status & UART_CH_STATUS_RTRIG){
		(void)xil_uart_getc();
	}
}